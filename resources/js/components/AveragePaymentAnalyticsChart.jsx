...
const AveragePaymentAnalyticsChart = ({ filter }) => {
    ...
    const fetchData = async () => {
        try {
            ...
            const res = await axios.get("/average-payment-analytics", { params });
            setData(res.data || []);
        } catch (err) {
            console.error("Ошибка загрузки данных средней оплаты:", err);
            setData([]);
        }
    };
    ...
    useEffect(() => {
            ...
            if (item.platforms?.length) {
                item.platforms.forEach((p) => {
                    if (base[p.platformType] !== undefined) {
                        base[p.platformType] = parseFloat(p.averagePaidAmount || 0);
                    }
                });
            }
            return base;
        ...
        const labels = transformed.map((d) => {
            if (multipleDays) {
                ...
                return `${monthName.charAt(0).toUpperCase() + monthName.slice(1)} ${date.getDate()} ${d.hour}`;
            }
            return d.hour;
        });
    ...
    return (
        <div className="card h-100">
            <div className="card-header d-flex flex-wrap justify-content-between align-items-center">
                <div className="fw-semibold">Средняя сумма оплаты</div>
...
export default AveragePaymentAnalyticsChart;

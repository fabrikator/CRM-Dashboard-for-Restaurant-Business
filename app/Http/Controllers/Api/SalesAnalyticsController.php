...
class SalesAnalyticsController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = DB::table('sales_analytics')
            ->select('stat_date', 'stat_hour', 'sales_amount', 'income_amount', 'customers_count', 'average_paid_amount', 'platform_data')
            ->orderBy('stat_date')
            ->orderBy('stat_hour');
        ...
        $result = $rows->map(function ($row) {
            return [
                ...
                'average_paid_amount' => (float)$row->average_paid_amount,
